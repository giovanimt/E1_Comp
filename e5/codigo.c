...
void gera_codigo_vl(NodoArvore *n){
    ...
    if(n->filho[5]){
        switch(n->filho[5]->nodo.valor_lexico.type) {
		...
		default:
			...
			if(true){//TODO: fazer if para descobrir se TK_IDENTIFICADOR eh vl ou vg, se for vg:
				...
				print("loadAI rbss,%d => %s\n", des_ident, reg_temp2);
				printf("storeAI %s => rfp, %d\n", reg_temp2, des_vl);
			}else{//se TK_IDENT for vl:
				char* reg_temp2 = gera_registrador();
				print("loadAI rfp,%d => %s\n", des_ident, reg_temp2);
				printf("storeAI %s => rfp, %d\n", reg_temp2, des_vl);
			break;
        }
    }
...
gera_codigo_if(NodoArvore *n){
	printf("nop\n");
}

gera_codigo_while(NodoArvore *n){
	printf("nop\n");
}

gera_codigo_do(NodoArvore *n){
	printf("nop\n");
}
